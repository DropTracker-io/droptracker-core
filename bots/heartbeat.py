...
BOT_RESTART_BASE_DELAY_SECONDS = 2
BOT_RESTART_MAX_DELAY_SECONDS = 60

# Health check function for systemd watchdog
async def health_check():
    """Comprehensive health check for the heartbeat bot"""
    try:
        # During startup, just check if bot is connected
        if not bot.is_ready:
            # Bot not ready yet, but that's okay during startup
            return True
        
        # Once bot is ready, we can do more comprehensive checks
        # But don't fail if webhook_states is still loading
        return True
...
async def run_bot_with_restarts() -> None:
    """
    Keep the Discord client running.
    If the gateway stack exits (e.g. transient transport reset), restart with backoff.
    """
    restart_attempt = 0
    while not shutdown_event.is_set():
        ...
        try:
            ...
            if shutdown_event.is_set():
                break
            print("Heartbeat bot client exited unexpectedly; preparing restart.")
        except asyncio.CancelledError:
            break
        except Exception as e:
            print(f"Heartbeat bot crashed, scheduling restart: {e}")
        finally:
            await shutdown_bot_client()

        # If we stayed up for a while, reset the backoff.
        ...
        delay = min(
            BOT_RESTART_MAX_DELAY_SECONDS,
            BOT_RESTART_BASE_DELAY_SECONDS * (2 ** min(restart_attempt, 5)),
        )
        jitter = random.uniform(0.0, 1.0)
        restart_in = delay + jitter
        print(f"Restarting heartbeat bot in {restart_in:.1f}s (attempt {restart_attempt})...")
        try:
            await asyncio.wait_for(shutdown_event.wait(), timeout=restart_in)
...
async def main():
    ...
    try:
        async with watchdog:
            ...
            # Start supervised bot loop; it self-restarts on transient disconnects.
            bot_task = asyncio.create_task(run_bot_with_restarts())
            shutdown_task = asyncio.create_task(shutdown_event.wait())

            # Wait for shutdown signal or unexpected supervisor exit.
            ...
            if bot_task in done and not shutdown_event.is_set():
                # The restart supervisor should not exit on its own.
                # Treat this as fatal so systemd can apply Restart policy.
                exc = bot_task.exception()
                if exc:
                    raise exc
                raise RuntimeError("Heartbeat bot supervisor exited unexpectedly")
