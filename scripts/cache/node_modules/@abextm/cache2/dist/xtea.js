...
export function decryptXTEA(data, key) {
    if (key.length != 4) {
...
function decryptXTEABlock(input, out, k, kIndex) {
    let v0 = input[0];
    let v1 = input[1];
    let sum = IV;
    for (let r = 0; r < ROUNDS; r++) {
...
export class XTEAKeyManager {
    unknownKeys = new KeySet();
    keysByMapSquare = new Map();
    allKeys = new KeySet();
    constructor() {
    }
    loadKeys(document) {
        ...
        if (Array.isArray(document)) {
            for (let item of document) {
                ...
                if (Array.isArray(item)) {
                    ...
                    // XTEAKey[]
                    this.unknownKeys.add(item);
                    if (this.allKeys.add(item)) {
                        count++;
                    }
                }
                else {
            ...
            for (let [mapsq, item] of Object.entries(document)) {
                if (Array.isArray(item)) {
                    ...
                    if (this.putKeyForMapsquare(~~mapsq, item)) {
                        count++;
                    }
                }
                else {
    ...
    putKeyForMapsquare(mapsquare, key) {
    ...
    tryDecrypt(ad, region) {
        ...
        for (let keyset of keys) {
            ...
            for (let index; (index = iterator()) != -1;) {
                if (data) {
                ...
                try {
                    ad.key = data ? Array.from(data.subarray(index, index + 4)) : undefined;
                    ad.getDecryptedData();
                    return undefined;
                }
                catch (e) {
