...
admin/dev path and the only mint path until then.
    ...
    ./venv/bin/python -m scripts.mint_api_key --global --label "partner site"
...
def main() -> int:
    ...
    owner.add_argument("--global", dest="is_global", action="store_true",
                       help="all-access key for a third-party integration "
                            "(every group, every player; hidden players still hidden)")
    ...
    try:
        if args.list:
            for row in session.query(ApiKey).order_by(ApiKey.id).all():
                ...
                scope = getattr(row, "scope", None) or (
                    "user" if row.owner_user_id is not None else "group")
                owner_desc = ("GLOBAL" if scope == "global"
                              else f"user {row.owner_user_id}" if row.owner_user_id is not None
        ...
        if not args.is_global and (args.user is None) == (args.group is None):
            parser.error("exactly one of --user / --group / --global is required")
        ...
        row, token = keys.create_key(
            ...
            group_id=args.group,
            scope="global" if args.is_global else None,
            label=args.label,
            tier_key=args.tier,
        )
        session.commit()
