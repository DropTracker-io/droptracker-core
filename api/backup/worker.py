...
import enum
from quart import Quart, request, jsonify, render_template, Blueprint, Response
...
# Enable CORS for this blueprint to allow requests from the web app
...
def _build_task_tile_image_png(task_id: int, task_name: str | None) -> bytes:
    """Generate a simple 100x100 PNG for a task tile.

    The API service intentionally avoids depending on the legacy `events` package.
    """
    size = (100, 100)
    ...
    # Border
    draw.rectangle([0, 0, size[0] - 1, size[1] - 1], outline=(90, 90, 90, 255), width=2)

    font = ImageFont.load_default()
    title = (task_name or "Unknown task").strip()
    if len(title) > 30:
        title = title[:27].rstrip() + "..."

    # Task id in the corner
    draw.text((6, 6), f"#{task_id}", fill=(220, 220, 220, 255), font=font)

    # Simple word-wrapping into up to 3 lines
    words = title.split()
    lines: list[str] = []
    current: list[str] = []
    for w in words:
        test = (" ".join(current + [w])).strip()
        if draw.textlength(test, font=font) <= 88:
            current.append(w)
        else:
            if current:
    ...
    for line in lines[:3]:
        ...
        x = max(6, (size[0] - line_w) // 2)
        draw.text((x, y), line, fill=(240, 240, 240, 255), font=font)
        y += 14
...
@worker_bp.route("/api/task_tile_image", methods=["GET"])
async def get_task_tile_image():
    """Return a PNG tile for a given task_id."""
    try:
        ...
        with get_db_session() as session:
            row = (
                session.execute(
                    sa.text("SELECT id, name FROM base_tasks WHERE id = :task_id LIMIT 1"),
                    {"task_id": task_id},
                )
                .mappings()
                .first()
        ...
        png = _build_task_tile_image_png(task_id=task_id, task_name=(row["name"] if row else None))
        resp = Response(png, mimetype="image/png")
        resp.headers["Cache-Control"] = "public, max-age=3600"
        return resp
    except Exception as e:
        # Avoid crashing the service if image generation fails
        print(f"Unexpected error in get_task_tile_image: {e}")
        png = _build_task_tile_image_png(task_id=0, task_name="Error")
        return Response(png, mimetype="image/png"), 500
...
async def get_task_tile():
    try:
        ...
        # Verify task exists (keeps behavior consistent: 404 if task is missing)
        with get_db_session() as session:
            exists = (
                session.execute(
                    sa.text("SELECT 1 FROM base_tasks WHERE id = :task_id LIMIT 1"),
                    {"task_id": task_id},
                )
                .first()
            )
        ...
        return jsonify({"tile_url": f"{base_url}/api/task_tile_image?task_id={task_id}"})
    except SQLAlchemyError as e:
        print(f"Database error in get_task_tile: {e}")
        return jsonify({"error": "Database error occurred"}), 500
    except Exception as e:
