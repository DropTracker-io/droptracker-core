"""
Incremental maintenance of the `player_npc_hourly_totals` rollup table.

The table powers the profile "top bosses" blocks (web_api/routes/profiles.py
`_top_bosses_sql`) but historically had no in-repo writer -- it went stale in
2025-08 when whatever external job populated it died. This module tails the
`drops` table by drop_id and folds new rows into the rollup, plus offers a
full per-partition rebuild for backfills.

State: Redis key `npc_totals:last_drop_id` holds the highest drop_id that has
...
LAST_ID_KEY = "npc_totals:last_drop_id"
...
# (player,npc,hour) bucket from `drops` and OVERWRITES the rollup (VALUES(),
# not col+VALUES()), with the `d.drop_id <= :max_id` cap that makes it safe to
# run on the CURRENT partition alongside the live additive tailer (see
# refold_day). An hour never spans two days, so every bucket is fully
# recomputed within its day chunk, making the upsert idempotent.
...
def process_new_drops(session=None, batch_size: int = 100_000, max_batches: int = 50) -> int:
    ...
    (scripts/backfill_npc_hourly_totals.py). Returns rows scanned.
    """
...
def rebuild_partition(partition: int, session=None, advance_pointer: bool = True) -> int:
    ...
    max_id = s.execute(text("SELECT COALESCE(MAX(drop_id), 0) FROM drops")).scalar()
    s.execute(
        text("DELETE FROM player_npc_hourly_totals WHERE `partition` = :partition"),
