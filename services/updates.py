...
from interactions import Extension, Permissions, slash_command, slash_option, OptionType, SlashContext, Embed
from interactions import SlashCommandChoice
from db.models import Session, User, UserConfiguration

class Updates(Extension):
    """Discord bot extension for managing update logs"""
    
    def __init__(self, bot: interactions.Client):
        self.bot = bot
        self.UPDATE_LOGS_DIR = "/store/droptracker/disc/data/updates"
        # Channel ID for updates - can be made configurable later
        self.channel_id_to_use = 1210765287591256084
    
    def get_update_file_path(self, version: str) -> str:
        """Get the file path for a specific update version"""
        return os.path.join(self.UPDATE_LOGS_DIR, f"update_{version.replace('.', '_')}.json")

    def load_update_data(self, version: str) -> dict:
        """Load update data from JSON file"""
        file_path = self.get_update_file_path(version)
        if os.path.exists(file_path):
            with open(file_path, 'r') as f:
                ...
                if "added" in data and isinstance(data["added"], list):
                    ...
                    self.save_update_data(version, data)
                return data
        return {
            "version": version,
            "categories": {},
    ...
    def save_update_data(self, version: str, data: dict) -> None:
    ...
    def list_update_versions(self) -> list:
        """List all available update versions"""
        if not os.path.exists(self.UPDATE_LOGS_DIR):
            return []
        versions = []
        for file in os.listdir(self.UPDATE_LOGS_DIR):
    ...
    async def update_log_message(self, ctx: SlashContext, input_str: str, version: str = None, publish: bool = False, should_ping: bool = False):
        """
        ...
        """
        channel_id = self.channel_id_to_use  # Your #updates channel ID
    ...
    @slash_option(name="type", description="Type of update item", opt_type=OptionType.STRING, required=True, choices=[
        SlashCommandChoice(name="Added", value="added"),
        SlashCommandChoice(name="Removed", value="removed"),
        SlashCommandChoice(name="Changed/Fixed", value="changed"),
        SlashCommandChoice(name="Note", value="notes")
    ])
    @slash_option(name="content", description="The content to add", opt_type=OptionType.STRING, required=True)
    async def update_add(self, ctx: SlashContext, version: str, category: str, type: str, content: str):
        ...
        try:
            # Load existing data
            data = self.load_update_data(version)
    ...
    async def update_view(self, ctx: SlashContext, version: str):
    ...
    @slash_command(name="update-list", 
                ...
                default_member_permissions=Permissions.ADMINISTRATOR)
    async def update_list(self, ctx: SlashContext):
        """List all available update log drafts"""
        try:
            versions = self.list_update_versions()
    ...
    async def update_send(self, ctx: SlashContext, version: str, publish: bool = False, should_ping: bool = False):
                ...
                try:
                    ...
                    with open(f"data/updates/to_send/{version}.txt", "w") as f: 
                        ...
                        for user_discord_id in user_discord_ids:
                            should_dm = local_session.query(UserConfiguration).filter(
                                UserConfiguration.user_id == user_discord_id[0], 
                                UserConfiguration.config_key == "dm_on_update_logs"
                            ).first()
    ...
    async def update_delete(self, ctx: SlashContext, version: str):
