...
**2026-09-10 — board styles + required tiles (shipped in code):** `settings.style` (`race` | `chutes_ladders`) is a designer/copy preset; the engine reads tiles. Tile links live in `EventBoardTile.config` (`{"jump_to": idx, "jump_when": "land"|"complete"}`): a landing-triggered link fires once inside the shared landing resolver (`boardgame_engine._resolve_landing`, used by rolls, teleport-advance AND knockback), a `complete` ladder lifts the team when its tile's task is done (`handle_board_completion`). Links never chain and a ladder may not cross a required tile (validated on save). The former `special` tile kind is now **`required`**: a move that would pass an uncleared required tile stops on it (`_required_stop`, cleared-ness read from the instance tasks' `config.tile_idx`); a finish tile that carries a difficulty/pin must be completed to win (`board["won"]`, consumer ends the event). `settings.win.exact_finish` (`off` | `stay` | `bounce`) governs overshooting rolls. Generator style `grid` renders the numbered boustrophedon board. Migration `web115a_board_required_tiles` (data-only) rewrites stored `special` rows; the read side aliases them meanwhile.
